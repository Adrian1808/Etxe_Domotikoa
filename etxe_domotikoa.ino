...
void setup() {
  ...
  RFID_abiarazi(&nfc);
  Serial.println("SALGO RFID");
  FPS_abiarazi(&fps);
  Serial.println("SALGO FPS");
  LSC_abiarazi(&camera);
...
*/
  ...
  rtc.setTime(40,43,07,1,23,1,17);
  ...
  //pinMode(18, INPUT_PULLUP); // Arduinoren PULL UP barne erresistentzia aktibatu
  pinMode(11,OUTPUT);
  pinMode(43, OUTPUT); //INPUT-1 (A)
  pinMode(45, OUTPUT); //INPUT-2 (B)
  miServo.attach(24, 650, 2400);
  pinMode(22, OUTPUT); 
  miServo.write(74);
...
void loop() {
  ...
  if(k_sarrera < 5) {
    ...
    if(p.z > __PRESURE) {
      ...
      if(p.x>160 && p.x<240) {
        if(p.y>30 && p.y<140){
          ETXEA_sarrera(&tft,&ts,&fps,&nfc,&rtc,data,&passwd,&k_passwd,&k_sarrera,&f_sarrera,&f_printed_konf_sarrera,&f_printed_sarrera,&t,&k_numero,&pertsiana,p.x,p.y, &miServo);
          Point p = ts.getPoint();
          p.x = map(p.x, TS_MINX, TS_MAXX, 0, 240);
          p.y = map(p.y, TS_MINY, TS_MAXY, 0, 320);
...
void Pertsianen_Kontrola(){
  ...
  if(month>5 && month<10){
    ...
    if((final<120 /*|| (hours==8 && minutes==0 && seconds==0))*/&& !pertsiana)){
      ...
      digitalWrite(45, LOW);
      analogWrite(11, 130); //PWM
      delay(150);
      analogWrite(11, 110); //PWM
      delay(250);
      analogWrite(11, 90); //PWM
      delay(510);
      analogWrite(11, 82); //PWM
      delay(400);
      analogWrite(11, 0); //PWM
      //Serial.print(day); Serial.print(", ");
      //Serial.print(date); Serial.print("/"); Serial.print(month); Serial.print("/"); Serial.print(year); Serial.print(", ");
      //Serial.print(hours); Serial.print(":"); Serial.print(minutes); Serial.print(":"); Serial.println(seconds);
      pertsiana=1;
    ...
    else if((final>248  || (hours==20 && minutes==0 && seconds==0))&& pertsiana){
      Serial.print("pertsiana jeitsi");
      //TFT_mezua(&tft, "JAISTEN...");
      digitalWrite(MOTORRA_A, LOW);
      digitalWrite(MOTORRA_B, HIGH);     
